...
import { updatePhotoCourse } from "@/api/course";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      if (file) {
        ...
        formPhoto.append("files", file);

        formPhoto.append("ref", "api::course.course");
        formPhoto.append("refId", course.id);
        formPhoto.append("field", "photo");

        const response = await updatePhotoCourse(formPhoto);

        if (response) {
          setPhotoCourse(response[0].url);
        }
      }
      ...
      <Form onSubmit={formik.handleSubmit}>
        <div className="content">
          ...
          <Grid>
            ...
            {photoCourse && (
              <Grid.Column width={8}>
                <Image src={photoCourse} alt="Blacktrainer" />
              </Grid.Column>
            )}
          </Grid>
